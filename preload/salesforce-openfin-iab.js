(function (host) {
  ...
  console.log("Loading OpenFin preload iab");
  ...
  function outbox(outgoingMessage) {
    const targetTopic = outgoingMessage.detail.topic ?? outboxTopic;
    ...
    if (outgoingMessage.detail.uuid !== undefined) {
      salesforcefin.InterApplicationBus.send(
        ...
        console.error(
          `Unable to publish message to application: ${outgoingMessage.detail.uuid} on topic: ${outboxTopic}. Reason: ${reason}`
        );
      });
    } else {
  ...
  function init() {
    salesforcefin.desktop.main(() => {
      ...
      setupInbox();
      setupUnsubscribe();
      console.log("Finished loading OpenFin preload iab");
    });
  }
